...
height = opt.image_size[0]
# channels = Config.channels
num_classes = opt.nclasses
# ctc_blank = Config.ctc_blank
device = opt.device

# IMG_ROOT = './crnn/data/part_300w/img'  # 测试图片存放文件夹
# IMG_ROOT = 'G:/jupyter/fragile_wm/img/random_sample'  # 测试图片存放文件夹
IMG_ROOT = 'G:/jupyter/fragile_wm/img/wrong_samples'

# 读取字典文件 用来翻译结果
fp = open(opt.char_dict_file, 'r', encoding='utf-8-sig')
samples = fp.read().splitlines()
labels = np.arange(num_classes).tolist()
char_dict = dict(zip(labels, samples))
fp.close()
...
def infer(image, model):
    ...
    # 对结果进行解码
    pred_labels = ctc_decode(output_log_softmax)
